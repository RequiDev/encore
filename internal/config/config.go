...
// albums, followed artists and playlists, and reconciles each against a local
// table. There is no delta endpoint for any of these, so every run is a full
// enumeration.
//
...
//
//	ceil(saved_tracks/50) + ceil(saved_albums/50) + ceil(followed/50) + 6 + ceil(playlists/50)
//
...
// of 50 is the whole picture by design, so unlike the other four that count
// never grows with the size of the library. The trailing ceil(playlists/50)
// is the playlist enumeration, paged the same 50-per-page way saved tracks,
// saved albums and followed artists already are. That is affordable, and a
